...
class Node_CameraFramePub():
    ...
    def run(self):
        while self.video_capture is not None and self.video_capture.isOpened():
            ...
            self.frame_ros = self.openCV_to_ros_image(frame)
            print("published")
            if ret:
                print("published")
                self.camera_frame_publisher.publish(self.frame_ros)
            time.sleep(0.0333333333)  # look into to reduce latency
    ...
    def select_camera(self, x):
        ...
        self.video_capture = cv2.VideoCapture("UI_App/src/test_video.mp4")
        # print(x.data)
        self.run()
