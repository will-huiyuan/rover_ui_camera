...
class Ui_MainWindow(QtWidgets.QWidget):
    ...
    def set_ui(self):
        ...
        self.button_open_camera1 = QtWidgets.QPushButton('打开相机1')  # 建立用于打开摄像头的按键
        self.button_open_camera2 = QtWidgets.QPushButton('打开相机2')  # 建立用于打开摄像头的按键
        self.button_close = QtWidgets.QPushButton('退出')  # 建立用于退出程序的按键
        self.button_open_camera1.setMinimumHeight(50)  # 设置按键大小
        self.button_open_camera2.setMinimumHeight(50)  # 设置按键大小
        ...
        '''把按键加入到按键布局中'''
        self.__layout_fun_button.addWidget(self.button_open_camera1)  # 把打开摄像头的按键放到按键布局中
        self.__layout_fun_button.addWidget(self.button_open_camera2)  # 把打开摄像头的按键放到按键布局中
    ...
    def slot_init(self):
        self.button_open_camera1.clicked.connect(
            self.button_open_camera1_clicked)  # 若该按键被点击，则调用button_open_camera1_clicked()
        self.button_open_camera2.clicked.connect(
            self.button_open_camera2_clicked)  # 若该按键被点击，则调用button_open_camera2_clicked()
    ...
    def button_open_camera1_clicked(self):
        ...
        if self.timer_camera.isActive() == False:  # 若定时器未启动
            flag = self.cap.open(CAM_NUM)  # 参数是0，表示打开笔记本的内置摄像头，参数是视频文件路径则打开视频
            ...
            self.button_open_camera1.setText('打开相机1')
    ...
    def button_open_camera2_clicked(self):
        ...
        if self.timer_camera.isActive() == False:  # 若定时器未启动
            ...
            if flag == False:  # flag表示open()成不成功
                ...
                self.timer_camera.start(30)  # 定时器开始计时30ms，结果是每过30ms从摄像头中取一帧显示
                self.button_open_camera2.setText('关闭相机')
        else:
            self.timer_camera.stop()  # 关闭定时器
            self.cap.release()  # 释放视频流
            self.label_show_camera.clear()  # 清空视频显示区域
            self.button_open_camera2.setText('打开相机2')

    def show_camera(self):
        flag, self.image = self.cap.read()  # 从视频流中读取sel
